...
const processDeclarations = ({ ctx, store }) => {
  ...
  const dec = declarations.reduce((acc, declaration) => {
    ...
    const processed = {
      ...declaration,
      situation: processSituations(declaration)
    };
    if (declaration.key) {
      _processDeclaration({ key: declaration.key, acc, processed });
    }
    if (declaration.keys) {
      declaration.keys.map(key => _processDeclaration({ key, acc, processed }));
    }
    return acc;
  }, {});
